...
#Version 1.2 adds in the feature of searching for a motif in the
#5' to 3' orientation and 3' to 5' at the same time on the 
#transcribed strand for each gene. It adds the two orientations together.
...
sub process_hash_plus {
	...
	my $motif_reverse = reverse $motif;
	#print "$motif_reverse\n";
	...
	foreach my $i ( sort{ $a<=>$b} keys %curr_hash) {
		...
		my $count2 =()= $curr_column =~ /$motif_reverse/gi;
		my $count3 = $count1 + $count2;
		print "$count3\,";
	}
	print "\n";
}
...
sub process_hash_minus {
	...
	foreach my $i ( sort{ $a<=>$b} keys %curr_hash) {
		...
		my $count2 =()= $curr_column1 =~ /$motif_reverse/gi;
		my $count3 = $count1 + $count2;
		print "$count3\,";
	}
	print "\n";
